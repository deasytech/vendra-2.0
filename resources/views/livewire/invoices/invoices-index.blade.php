        ...
        <div class="bg-white rounded-xl shadow-lg p-4 border-l-4 border-blue-500">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <div class="text-2xl font-bold text-gray-800 space-y-1">
                        @php
                            $currencySymbols = [
                                'NGN' => '₦',
                                'USD' => '$',
                            ...
                            $totalsByCurrency = $invoices->getCollection()
                                ->groupBy('document_currency_code')
                                ->map(fn ($group) => $group->sum(fn ($invoice) => $invoice->legal_monetary_total['payable_amount'] ?? 0));
                        @endphp
                        @forelse ($totalsByCurrency as $currencyCode => $total)
                            <p>{{ $currencySymbols[$currencyCode] ?? $currencyCode.' ' }}{{ number_format($total, 2) }}</p>
                        @empty
                            <p>₦0.00</p>
                        @endforelse
                    </div>
